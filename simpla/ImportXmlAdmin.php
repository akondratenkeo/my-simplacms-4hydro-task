...
{	
	...
	public $import_file_suf = 'import_';
	public $allowed_extensions = array('xml', 'xlsx');
	...
	public function fetch()
	{
		...
		if($this->request->method('post') && ($this->request->files("file")))
		{
            ...
            $f_name = $this->request->files("file", "name");
            $f_size = filesize($uploaded_name);
            $f_import_name = $this->import_file_suf . dechex(time()) . $this->get_extension($f_name, 1);
            $f_error = 0;

            if($uploaded_name != '') {
                // Проверяем формат файла
                if (!in_array($this->get_extension($f_name), $this->allowed_extensions)) {
                    $this->design->assign('message_error', 'format_error');
                    $f_error++;
                }

                // Проверяем размер файла
                if ($f_size > 131072000) {
                    $this->design->assign('message_error', 'size_error');
                    $f_error++;
                }
            }else{
                $this->design->assign('message_error', 'no_file_error');
                $f_error++;
            }
			...
			if($f_error == 0) {
                ...
                if (!$this->convert_file($temp, $this->import_files_dir . $f_import_name, $this->get_extension($f_import_name)))
                    $this->design->assign('message_error', 'convert_error');
                else
                    $this->design->assign('filename', $f_name);
                    $this->design->assign('f_import_name', $f_import_name);
                unlink($temp);
            }
		}
	...
	private function convert_file($source, $dest, $filetype)
	{
